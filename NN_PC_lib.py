...
def create_imagenet_valid_dataset(imgpix=256, normalize=True,):
    ...
    dataset = ImageFolder(r"C:\Users\Li\Research\Harvard_Group\Visual_Neuro_InSilico_Exp\ImageNet_data\imagenet_50K_val", transform=preprocess)
    return dataset
...
def record_dataset(model, reclayers, dataset, return_input=False,
    ...
    # switch device here
    fetcher = featureFetcher(model, device="cpu", store_device="cpu", print_module=False)
    for layer in reclayers:
        fetcher.record(layer, return_input=return_input, ingraph=False)
    # a dictionary, layer: a list of features of all images
...
def featdir_GAN_visualize(G, CNNnet, layername, objfunc, tfms=[], imgfullpix=256, imcorner=(0, 0), imgpix=None,
    ...
    for step in pbar:
        ...
        if langevin_eps > 0.0:
            ...
            z.data.add_(torch.randn(z.shape, device="cpu") * langevin_eps)
...
def get_RF_location(model, layer, cent_pos, imgfullpix=256):
    ...
    gradAmpmap = grad_RF_estimate(model, layer, (slice(None), *cent_pos), input_size=(3, imgfullpix, imgfullpix),
                              device="cpu", show=False, reps=30, batch=1)
...
def get_cent_pos(model, layer, imgfullpix=256):
    module_names, module_types, module_spec = get_module_names(model, (3, imgfullpix, imgfullpix), device="cpu", show=False)
...
# switch device here
def test_record_layernames(model, reclayers, return_input=False, device="cpu"):
